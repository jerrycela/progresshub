...
### 迭代檢視重點
- **TypeScript 規範** 🔴：未使用變數、類型定義、嚴格模式相容性
- **程式碼品質**：可讀性、命名、結構
- **效能優化**：減少重複計算、優化渲染
...
### 迭代完成前必做檢查 🔴 Critical
每次迭代完成後、提交前，**必須**執行以下檢查：

```bash
# 前端 TypeScript 檢查
cd packages/frontend && npx vue-tsc --noEmit

# 後端 TypeScript 檢查（如有修改）
...
| 錯誤碼 | 說明 | 解決方案 |
|--------|------|----------|
| TS6133 | 未使用的變數 | 移除或確認模板使用 |
| TS2345 | 類型不匹配 | 檢查類型定義 |
| TS2339 | 屬性不存在 | 確認介面定義 |

**Vue 特別注意**：
- `<script setup>` 中的變數若用於模板，vue-tsc 可能無法偵測
- 若確定變數有在模板使用但仍報錯，可用 `_templateRefs` 技巧標記
