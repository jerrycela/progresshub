...
defineProps<{
  task: Task
  index: number
  getTaskPosition: (task: { startDate?: string; dueDate?: string }) => { left: number; width: number }
...
<template>
  ...
  >
    ...
    <div class="w-28 sm:w-40 flex-shrink-0 pr-2">
      ...
      <div v-if="showProject && getProjectName" class="text-xs truncate mt-0.5" style="color: var(--text-muted);">
    ...
    <div class="flex-1 relative h-7 hidden sm:block">
      <div
        ...
        :class="[
          isTaskOverdue(task) ? 'ring-2 ring-danger/50 bg-danger' : (statusColors[task.status] || 'bg-gray-500'),
      ...
      >
        <!-- 暫停圖示 -->
        <svg v-if="task.status === 'PAUSED'" class="w-3 h-3 mr-0.5" fill="currentColor" viewBox="0 0 24 24">
          <path d="M6 4h4v16H6V4zm8 0h4v16h-4V4z" />
        </svg>
        <!-- 逾期圖示 -->
        <svg v-else-if="isTaskOverdue(task)" class="w-3 h-3 mr-0.5" fill="currentColor" viewBox="0 0 24 24">
        ...
        <!-- Tooltip -->
        <div class="absolute bottom-full left-0 mb-2 hidden group-hover:block z-20 pointer-events-none">
          <div class="p-2 rounded-lg shadow-lg text-xs whitespace-nowrap" style="background-color: var(--bg-tooltip); color: var(--text-on-tooltip, #fff);">
            ...
            <div v-if="getDaysRemaining(task) !== null" :class="getDaysRemaining(task)! < 0 ? 'text-danger' : 'text-success'">
              {{ getDaysRemaining(task)! < 0 ? `逾期 ${Math.abs(getDaysRemaining(task)!)} 天` : `剩餘 ${getDaysRemaining(task)} 天` }}
            </div>
          </div>
        </div>
      </div>
