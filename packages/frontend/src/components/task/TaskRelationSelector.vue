...
const availableTasks = computed(() =>
  getAvailableTasks(props.allTasks, props.currentTaskId)
)
...
const filteredTasks = computed(() => {
  ...
  return availableTasks.value.filter((task: Task) =>
  ...
  <div class="relative">
    ...
    >
      ...
      <div class="max-h-64 overflow-y-auto">
        <div v-if="groupedTasks.length === 0" class="p-4 text-center text-sm" style="color: var(--text-tertiary);">
          無符合的任務
        </div>

        <div v-for="group in groupedTasks" :key="group.projectId" class="border-b last:border-b-0" style="border-color: var(--border-secondary);">
          <!-- 專案名稱 -->
          <div class="px-3 py-2 text-xs font-semibold" style="color: var(--text-tertiary); background-color: var(--bg-tertiary);">
            {{ group.projectName }}
          </div>
          ...
          >
            ...
            <div
              ...
              :class="isSelected(task.id) ? 'bg-[var(--accent-primary)] border-[var(--accent-primary)]' : 'border-[var(--border-primary)]'"
            >
              <svg
