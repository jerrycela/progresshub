...
const overdueTasks = computed(() =>
  (taskStore.tasks as Task[]).filter(
    (t: Task) => t.dueDate && t.dueDate < today && t.status !== 'DONE',
  ),
)
...
const longUnclaimedTasks = computed(() => {
  ...
  return (taskStore.tasks as Task[]).filter(
    (t: Task) =>
      t.status === 'UNCLAIMED' && t.createdAt && t.createdAt.split('T')[0] < thresholdDate,
  )
})
      ...
      <div class="divide-y" style="border-color: var(--border-primary)">
        ...
        >
          <div class="flex-1 min-w-0">
            ...
            <div
              class="flex flex-wrap items-center gap-2 sm:gap-3 mt-1 text-sm"
          ...
          <div class="flex items-center gap-2 flex-wrap">
            <Badge v-for="func in task.functionTags" :key="func" variant="default" size="sm">
      ...
      <div class="divide-y" style="border-color: var(--border-primary)">
        ...
        >
          ...
          <div class="flex items-center gap-2">
            <Badge v-for="func in task.functionTags" :key="func" variant="info" size="sm">
              {{ functionTypeLabels[func] }}
            </Badge>
          </div>
        </div>
