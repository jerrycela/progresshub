...
const canManageMilestones = computed(() =>
  ['PRODUCER', 'MANAGER'].includes(currentUser.userRole)
)
...
// 使用 Gantt composable
const {
  dateRange, todayPosition, timeAxisMarks,
  getTaskPosition, getMilestonePosition,
  getTaskDuration, getDaysRemaining,
  tasksOutsideRange, formatDate,
...
const isProjectCollapsed = (projectId: string): boolean =>
  collapsedProjects.value.has(projectId)
...
// 里程碑 CRUD
const submitMilestone = (data: { name: string; description: string; date: string; projectId: string; color: string }): void => {
  if (!data.name.trim()) {
...
<template>
  <div class="space-y-6">
    ...
    <GanttFilters
      ...
      v-model:groupByProject="groupByProject"
