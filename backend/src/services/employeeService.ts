...
export class EmployeeService {
   ...
   * 取得員工列表（分頁）
   */
  async getEmployees(params: EmployeeListParams): Promise<{ data: Employee[]; total: number }> {
    const { page = 1, limit = 20, department, permissionLevel, search } = params;
   ...
   */
  async slackUserIdExists(slackUserId: string, excludeId?: string): Promise<boolean> {
    const employee = await prisma.employee.findUnique({
      where: { slackUserId },
    });
    return employee !== null && employee.id !== excludeId;
