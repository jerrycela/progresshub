...
router.post(
  ...
  [
    ...
    body('plannedStartDate').isISO8601().withMessage('Valid start date is required'),
    body('plannedEndDate').isISO8601().withMessage('Valid end date is required'),
  ...
  async (req: AuthRequest, res: Response): Promise<void> => {
    ...
    try {
      // 驗證日期
      if (new Date(req.body.plannedEndDate) <= new Date(req.body.plannedStartDate)) {
